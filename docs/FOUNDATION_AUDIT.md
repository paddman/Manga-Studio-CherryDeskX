...
| Document | Volume → Chapter → Page create, duplicate, delete and drag reorder; B5/A5/comic/A4/Webtoon/custom presets; publishing metadata and an explicit CMYK prepress warning | CMYK remains document metadata; browser Canvas and local exports are RGB/sRGB |
...
| Raster | Full-page layers, parameterized brush/eraser/fill/gradient/shape primitives, masks, alpha lock, split latest stroke, tones/effect lines, replayable Blur/Sharpen/Dodge/Burn/Sponge/Red Eye brushes and guarded local boundary-propagation Content-Aware Fill | Several named presets intentionally share a tested raster primitive; vector editing, semantic inpainting, healing/clone retouch and frequency separation are not emulated |
| Rulers | Straight ruler projection and replayable symmetry-axis brush mirroring with visible non-exporting guides | Curve, figure, parallel, radial and full perspective rulers remain disabled until their constraint solvers exist |
| Lettering | Horizontal/vertical text, font/weight/alignment/spacing/outline/shadow, auto-fit, reusable style presets, six balloon variants, multiple tails, validated TTF/OTF/WOFF embedding and basic Japanese kinsoku wrapping shared by auto-fit/export | Font subsetting/licence analysis, full JIS X 4051 line composition, text-on-path and text warp remain follow-up work |
| Persistence | IndexedDB project/image/font/raster repositories, localStorage data-URL migration, schema v6, validated `.cherrymanga` round trip | Cross-device sync and revision conflict handling require CherryDeskX Workspace APIs |
| Export | PNG/JPG/PDF/CBZ/ZIP/Webtoon, page/chapter/volume/project scope, 1×/2×/300-DPI/custom scale, bleed/crop marks, progress/cancellation and worker-based ZIP/CBZ/PDF packaging | Canvas/font/image composition remains on the main thread; a full OffscreenCanvas or server renderer remains an adapter |

...
describe("typed tool registry", () => {
  ...
  it("never enables a tool without a registered interaction engine", () => {
    ...
    expect(TOOL_DEFINITIONS.find((tool) => tool.id === "magic-wand")?.capability).toBe("experimental");
    expect(TOOL_DEFINITIONS.find((tool) => tool.id === "magic-wand")?.engine).toBe("selection-pixels");
    expect(TOOL_DEFINITIONS.find((tool) => tool.id === "quick-selection")?.engine).toBe("selection-pixels");
    ...
    expect(TOOL_DEFINITIONS.find((tool) => tool.id === "panel-cutter")?.engine).toBe("panel-split");
  ...
  it("resolves the standard keymap and supports a typed custom keymap", () => {
    ...
    expect(resolveToolShortcut("q", { Q: toolId("magic-wand") })).toBe(toolId("magic-wand"));
  });
});
